...
def send_post_request_config(host='shard_manager', port=5000, path='/config'):
    print("/config")
    # payload = {
    #     "N":3,
    #     "schema":{
...
def send_get_request_heartbeat(host='shard_manager', port=5000, path='/heartbeat'):
    print("/heartbeat")
    connection = http.client.HTTPConnection(host, port)
...
def send_get_request_copy(host='shard_manager', port=5000, path='/copy'):
    print("/copy")
    payload = {
        "shards": ["sh1", "sh2"]
    }
...
def send_post_request_read(host='shard_manager', port=5000, path='/read'):
    print("/read")
    payload = {
        "shard":'sh1',
...
def send_post_request_write(index=0,host='shard_manager', port=5000, path='/write'):
    print("/write")
    payload = {
        "shard":"sh1",
        # "curr_idx":2,
...
def send_put_request_update(index=0,host='shard_manager', port=5000, path='/update'):
    print("/update")
    payload = {
        "shard":"sh1",
        "Stud_id": index,
...
def send_put_request_make_primary(host='shard_manager', port=5000, path='/set_primary'):
    print("/make_primary")
    payload = {
        "shard":"sh1",
        "secondary_servers": ["load_balancer"]
    }
    headers = {'Content-type': 'application/json'}
    json_payload = json.dumps(payload)
...
def send_delete_request_del(index=0,host='shard_manager', port=5000, path='/del'):
    print("/del")
    payload = {
        "shard":"sh1",
        "Stud_id": index
...
def send_updateidx(index = 2, host='shard_manager', port=5000, path='/updateid'):
    print("/updateid")
    payload = {
        "shard":"sh1",
        "update_idx":index
